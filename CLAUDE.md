...
`scripts/runner.py`'s `Runner` class drives one benchmark invocation end-to-end (spawn `lock_exe` subprocess → parse binary log → export parquet); `run_permutations()` sweeps a param-space grid × `reps` iterations, writing to `files/logs/<dir_id>/` (raw binary) and the parquet tree at `<csv_dir>/<dir_id>/`, and returns `(dir_ids, saturated)` — the `dir_id`s it produced so the caller can scope the analysis stages, plus the names of any runs whose log buffer filled.
