...
```bash
./build/bin/lock_exe [num_threads] [core_pin_policy] [lock_type] [iterations] [filename(optional)]
```

- `core_pin_policy`: `0` = no pinning, `1` = round-robin, `2` = all threads on one core, `3` = half on one hot core / half round-robin.
- `lock_type`: `mcs`, `clh`, `ticket`, `ttas`, `ttasb`, `tsspin` (must match a branch in `src/runner/main.cpp`'s lock-type dispatch — new locks need a line added there).
- `iterations`: number of loop iterations of simulated work inside the critical section.
- Run with all args or no args (defaults: 8 threads, round-robin pin, MCS, 1000 iterations).
- `DURATION`/`WARMUP`/`CAPACITY` (10s run, 3s warmup, ~67M log entries/thread) are compile-time constants in `main.cpp`.
...
This calls, in order: `scripts.runner.run_permutations()` (drives `build/bin/lock_exe` across the lock/thread-count/pin-policy grid defined in `scripts/runner.py`'s `param_space`, 10 iterations each), `analysis` metric averaging, then single-run and cross-run plotting. There's no test suite for the Python code either; sanity-check by running against a small param space and inspecting the output CSVs/figures.
...
`scripts/runner.py`'s `Runner` class drives one benchmark invocation end-to-end (spawn `lock_exe` subprocess → parse binary log → export parquet); `run_permutations()` sweeps the full `param_space` grid × 10 iterations, writing to `files/logs/<lock>_<threads>_<pin>/` (raw binary) and `files/csv/<lock>_<threads>_<pin>/` (parquet).
